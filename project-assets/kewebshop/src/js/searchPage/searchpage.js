...
$(function () {
    ...
    $(document).on("click", '.js-cat', function (e) {
        ...
        getProductsByCategory(thisId, searchQuery);
        getProductsFilters(thisId, searchQuery);
    });

    async function getProductsByCategory(id, searchQuery) {
        await $.ajax({
            type: 'POST',
    ...
    async function getProductsFilters(id, searchQuery) {
        await $.ajax({
            type: 'POST',
            url: myObj.restURL + 'baseUrl/v1/baseEndPoint/getsearchfilters',
            data: {
                productCat: id,
                searchQuery: searchQuery
            },
            success: function (response) {
                if (response) {
                    $('.js-search-header').html(response);
                }
            }
        });
    }
...
async function getProductsByFilterOptions() {
    ...
    if ($('.js-cat.highlight--text').length) {
        activeCat = $('.js-cat.highlight--text').parents('li').data('cat');
    } else {
        activeCat = [];
        $('.js-cat').each(function (index) {
            const thisId = $(this).parents('li').data('cat');
            activeCat.push(thisId)
        });
    }
